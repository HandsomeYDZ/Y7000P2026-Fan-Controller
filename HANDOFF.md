...
> **2026-09-05 后续核查更新：原文多处结论已被修正。请先读 [普通模式控制核查](diag/NORMAL-MODE-FINDINGS.md)。本机均衡模式已通过独立 WMI 目标 RPM 接口实测调速，不必进入 Custom。CPU/GPU 的 Capability=7；目标范围 1700–5300 RPM；SetFeatureValue 的 0 表示 auto。当前已实现三普通模式的软件曲线控制，但真实硬件只验证了均衡短测，其他两档及长期控制待验证。原黑屏原因仍未定；“EC 地址动态漂移”“全最低表安全恢复”都不能沿用为事实。下文保留为历史记录。**
